...
@router.get("/users/collaborators", response_model=List[schemas.UserResponse])
def read_collaborators(db: Session = Depends(auth.get_db), user: models.User = Depends(allow_admin_only)):
